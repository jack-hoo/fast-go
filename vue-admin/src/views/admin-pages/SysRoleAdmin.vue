...
<script>
    import {
        add_sys_role as add_my_sys_role, get_sys_role_list, delete_sys_role, update_sys_role,
    } from '../../api/SysRoleApi';
    ...
    export default {
        ...
        methods: {
            update_sys_role() {
                update_sys_role(this.sysRole).then(res => {
            ...
            delete_sys_role(index) {
                let ids = [];
                ids.push(this.sysRoleList[index].id);
            ...
            get_sys_role_list() {
                //如果角色是管理员则调用管理员的接口
                get_sys_role_list(this.params).then(res => {
